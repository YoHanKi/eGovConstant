...
- **강력한 검색 및 랭킹**: 용어(Terms), 단어(Words), 도메인(Domains) 통합 모델을 지원하며, 가중치 기반 스코어링 엔진을 통해 정확도 높은 추천을 제공합니다. (동의어 및 퍼지 검색 지원)
- **변수명 생성 규칙**: 일관된 약어 처리 및 접미사 규칙을 적용하여 camelCase, snake_case, PascalCase 등 다양한 형식을 제공합니다.
- **eGovConstant 도구 창**: 데이터셋 관리, 검색, 복사 및 삽입 기능을 제공하며 페이지당 결과 표시 개수를 조절할 수 있습니다.
- **코드 자동 완성**: 변수 선언 시 2글자 이상 입력하거나 camelCase 중간에 대문자를 입력하면 표준 용어를 기반으로 이름을 추천합니다. (Java, Kotlin 지원)
